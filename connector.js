 ...
 */
 sendRequest(callOptions, callback) {
  ...
  this.callOptions = this.options;
  let uri;
  if (this.callOptions.query)
    uri = this.constructUri(this.callOptions.serviceNowTable, this.callOptions.query);
  else
    uri = this.constructUri(this.callOptions.serviceNowTable);
  /**
